...
NSTI_labs <- data.frame(fread("NSTI_labs_for_KL.csv", header = TRUE))
...
```{r fig.width= 60, fig.height= 30}
# creates a df for a outputting it in a nested list format
...
for (i in nsti_new$Patient){
  if ((which(nstimeds_new$study_ID == i) %>% is_empty()) != TRUE){
    ...
    if (tempmednum != 0){
      medwane = rbind(medwane, data.frame("med" = c(""), patient = as.integer(i), daycount = as.integer(daycount+1)))
    }
  }
}
...
medwaneadmit <- data.frame(patient = integer(), status = integer(), daycount = integer())
for (i in nsti_new$Patient){
  if ((which(nstimeds_new$study_ID == i) %>% is_empty()) != TRUE){
    temp <- nstimeds_new[which(nstimeds_new$study_ID == i,),]
    temptime <- temp[order(temp["totalTime"]),]
    admission <- (nsti_new %>% subset(Patient == i))$ADMIT.DATE
    end <- tail(temptime,1)$totalTime
    discharge <- (nsti_new %>% subset(Patient == i))$Discharge
    
    if (discharge > end){
      medwaneadmit <- rbind(medwaneadmit, data.frame(patient = i, status = 1,daycount = difftime(end,admission)))
    } else {
      medwaneadmit <- rbind(medwaneadmit, data.frame(patient = i, status = 0,daycount = difftime(discharge,admission)))
    }
  }
}
...
#survival Curve 
fit <- surv_fit(Surv( time = daycount, event= status) ~1, data = medwaneadmit, conf.type = "log-log") 
#autoplot(fit)
...
NSTI_labs %>% subset(Lab.Code == "WBC") %>% subset(TRACKING.NO == 1)
